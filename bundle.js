...
/***/ function(module, exports, __webpack_require__) {
	...
	var billEvents = __webpack_require__(8);
...
/***/ function(module, exports, __webpack_require__) {
	...
	//
	var onSignInView = function onSignInView(event) {
	  event.preventDefault();
	  ui.showSignInView();
	};
	...
	var onMainMenuView = function onMainMenuView(event) {
	  event.preventDefault();
	  ui.showMainMenuView();
	};

	//
	var onChangePasswordView = function onChangePasswordView(event) {
	  event.preventDefault();
...
/***/ function(module, exports, __webpack_require__) {
	...
	var showLandingView = function showLandingView() {
	  clearView();
	  $('#navigation-bar').hide();
	  $('#footer').hide();
	  $('#landing-view').show();
	...
	var showSignInView = function showSignInView() {
	  clearView();
	  $('#sign-in-view').show();
	};

	var clearBillInfoFromApp = function clearBillInfoFromApp() {
	  app.bill = null;
	...
	var showMainMenuView = function showMainMenuView() {
	  clearView();
	...
	var showChangePasswordView = function showChangePasswordView() {
	  clearView();
	  $('#change-password-view').show();
	};

	// Parse the user's email to get a working username
	var storeUserName = function storeUserName() {
	  var email = app.user.email;
	  var stop = email.indexOf('@');
	  var userName = email.charAt(0).toUpperCase() + email.slice(1, stop);
	...
	module.exports = {
	  ...
	  showMainMenuView: showMainMenuView,
...
/***/ function(module, exports, __webpack_require__) {
	...
	//
	var onLookUpBillView = function onLookUpBillView(event) {
	  event.preventDefault();
	  ui.showLookUpBillView();
	};

	//
	var onIndexBillsView = function onIndexBillsView(event) {
	  event.preventDefault();

	  api.indexBills().done(ui.indexBillsSuccess).fail(ui.indexBillsFailure);
	};

	//
	var onGetBillById = function onGetBillById(event) {
	  event.preventDefault();
	  var data = getFormFields(event.target);
	  var id = data.bill.id;

	  api.showBill(id).done(ui.showBillSuccess).fail(ui.showBillFailure);
	};

	//
	var submitShareInfo = function submitShareInfo() {
	  /*
	...
	//
	var submitBillInfo = function submitBillInfo() {
	  var data = {};
	  data.bill = app.currentBill;
	  api.createBill(data).done(ui.createBillSuccess).done(submitShareInfo).done(ui.showWorkingShareSummaryView).fail(ui.createBillFailure);
	};

	//
	var onGetNamesView = function onGetNamesView(event) {
	  event.preventDefault();
	  ui.showGetNamesView();
	};

	// Parses name submissions into an array of names and stores array on app.
	var onSubmitGroupNames = function onSubmitGroupNames(event) {
	  event.preventDefault();
	  var data = getFormFields(event.target);
	...
	//
	var onSubmitTotalAmount = function onSubmitTotalAmount(event) {
	  event.preventDefault();
	...
	//
	var setUpDynamicNameFields = function setUpDynamicNameFields() {
	  var next = 1;
	  $(".add-more").click(function (event) {
	    ...
	    var newIn = '<input autocomplete="off" class="name-input form-control" id="field' + next + '" name="names[' + next + ']" type="text">';
	    ...
	    var removeBtn = '<button id="remove' + (next - 1) + '" class="btn form-control remove-me" >-</button></div><div id="field">';
	...
	//
	var onChangeTotalAmountView = function onChangeTotalAmountView(event) {
	  event.preventDefault();
	  ui.showChangeTotalAmountView();
	};

	//
	var onChangeTotalAmount = function onChangeTotalAmount(event) {
	  event.preventDefault();
	  var data = getFormFields(event.target);
	  var totalAmount = data.bill.total_amount;
	  app.bill.total_amount = totalAmount;

	  api.updateBill(totalAmount).done(ui.updateBillAmountSuccess).done(submitShareInfo).done(ui.showWorkingShareSummaryView).fail(ui.updateBillAmountFailure);
	};

	//
	var onDeleteNewBill = function onDeleteNewBill(event) {
	  event.preventDefault();
	  var id = app.bill.id;
	  api.deleteBill(id).done(ui.deleteBillSuccess).fail(ui.deleteBillFailure);
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var indexBills = function indexBills() {
	  var token = app.user.token;
	  return $.ajax({
	    url: app.host + '/bills',
	    method: 'GET',
	...
	var showBill = function showBill(id) {
	  var token = app.user.token;
	  return $.ajax({
	...
	var updateBill = function updateBill(totalAmount) {
	  var id = app.bill.id;
	  var token = app.user.token;
	  return $.ajax({
	    url: app.host + '/bills/' + id,
...
/***/ function(module, exports, __webpack_require__) {
	...
	var showLookUpBillView = function showLookUpBillView() {
	  clearView();
	  $('#look-up-bill-view').show();
	};

	var showIndexBillsView = function showIndexBillsView() {
	  clearView();
	  $('#index-bills-view').show();
	};

	var showShowBillView = function showShowBillView() {
	  clearView();
	  $('#show-bill-view').show();
	};

	// Display view for user to enter names of group members
	var showGetNamesView = function showGetNamesView() {
	  clearView();
	  $('#get-group-names-view').show();
	};

	//
	var showGetTotalAmountView = function showGetTotalAmountView() {
	  clearView();
	  $('#get-total-amount-view').show();
	};

	// Write new HTML for an individual share
	var displayShareTemplate = function displayShareTemplate(share) {
	  var shareHMTL = '<p>' + share.name + ' owes $' + share.amount + '</p>';
	  return shareHMTL;
	};

	// Display new HTML for an individual share
	var createShareHTMLFromTemplate = function createShareHTMLFromTemplate(share) {
	...
	var displayShares = function displayShares() {
	  var shares = app.shares;
	  shares.forEach(createShareHTMLFromTemplate);
	};

	// Display a summary of the current share breakdown
	var showWorkingShareSummaryView = function showWorkingShareSummaryView() {
	  clearView();
	  $('#working-share-summary-view').show();
	};

	//
	var showChangeTotalAmountView = function showChangeTotalAmountView() {
	  clearView();
	...
	// Display new HTML for an individual share
	var addBillHMTLtoView = function addBillHMTLtoView(bill) {
	...
	var displayBills = function displayBills(bills) {
	  bills.forEach(addBillHMTLtoView);
	};

	var clearBillsFromView = function clearBillsFromView() {
	  $('#bills-list').find('p').remove();
	};

	//
	var indexBillsSuccess = function indexBillsSuccess(data) {
	...
	//
	var indexBillsFailure = function indexBillsFailure(error) {
	  console.error(error);
	};

	//
	var showBillSuccess = function showBillSuccess(data) {
	  var bill = data.bill;
	...
	//
	var showBillFailure = function showBillFailure(error) {
	  console.error(error);
	};
	...
	var clearSharesFromView = function clearSharesFromView() {
	  $('#shares-list').find('p').remove();
	};

	// Respond to a successful bill update amount attempt
	var updateBillAmountSuccess = function updateBillAmountSuccess() {
	  // clear shares from share summary
	  clearSharesFromView();
	};

	// Respond to a failed bill update amount attempt
	var updateBillAmountFailure = function updateBillAmountFailure(error) {
	  console.error(error);
	};
	...
	module.exports = {
	  showMainMenuView: showMainMenuView,
	  showLookUpBillView: showLookUpBillView,
	  showGetNamesView: showGetNamesView,
	  showGetTotalAmountView: showGetTotalAmountView,
	  displayShares: displayShares,
	  showWorkingShareSummaryView: showWorkingShareSummaryView,
	  showChangeTotalAmountView: showChangeTotalAmountView,
	  clearSharesFromView: clearSharesFromView,
	  indexBillsSuccess: indexBillsSuccess,
	  indexBillsFailure: indexBillsFailure,
	  showBillSuccess: showBillSuccess,
	  showBillFailure: showBillFailure,
	  createBillSuccess: createBillSuccess,
	  createBillFailure: createBillFailure,
	  updateBillAmountSuccess: updateBillAmountSuccess,
	  updateBillAmountFailure: updateBillAmountFailure,
...
/***/ function(module, exports, __webpack_require__) {
	...
	var filterBlankNames = function filterBlankNames(names) {
	  return names.filter(function (name) {
	    return name;
	  });
	};

	var processNames = function processNames(data) {
	  var names = [app.userName];
	...
	var createShares = function createShares() {
	  var names = app.names;
	  var shares = [];
	  var totalAmount = app.bill.total_amount;
	  var numPeople = app.bill.num_people;
	  var amountPerShare = (totalAmount / numPeople).toFixed(2);
